...
const ZEN_PROXY_REEXPORT = `// one-shot-ai-panel — OpenCode Zen same-origin relay (compiled mode).
// Generated by \`one-shot-ai-panel install --proxy-only\`. The relay lives in the
// package dist (\`one-shot-ai-panel/zen-proxy\`), so it always matches the installed
// version — no file to copy or keep in sync.
...
Options:
  ...
  --proxy-only     Compiled mode: only generate the Zen same-origin proxy as a one-line re-export
                   of the package's bundled handler (keep module, ui/*, css and deps untouched)
...
function parseArgs(argv) {
  const args = { dir: process.cwd(), force: false, noInstall: false, noCss: false, noRegistry: false, pm: null, moduleDir: "src/external-modules/ai-panel", moduleOnly: false, proxyOnly: false, noProxy: false }
  const rest = [...argv]
...
// Compiled-mode Zen proxy: instead of copying the relay source into the app,
// write a one-line re-export of the versioned handler shipped in the package
// dist ("one-shot-ai-panel/zen-proxy"), so it stays in sync with the installed
// package version automatically.
function installZenProxyCompiled(target, force) {
  if (args.noProxy) {
    log("•", "Zen same-origin proxy skipped (--no-proxy)")
    return false
  }
  ...
  if (framework === "next") {
    ...
    mkdirSync(dirname(routeFile), { recursive: true })
    writeFileSync(routeFile, ZEN_PROXY_REEXPORT)
    log("✓", `generated Zen proxy (re-export of one-shot-ai-panel/zen-proxy): ${relative(target, routeFile)}`)
    return true
  }
...
if (args.proxyOnly) {
  installZenProxyCompiled(target, args.force)
  log("", "proxy-only mode: module, ui/*, css and deps are left untouched.")
  process.exit(0)
}
